...
class AllocationApiIntegrationTest extends BaseIntegrationTest {
    ...
    void testUpdateAllocationDoesNotExists() {
        assertThrows(
                HttpClientErrorException.NotFound.class,
                () -> api.updateAllocation(TEST_CLIENT_API_KEY, 1L, new UpdateAllocationDTO())
        );
    }
    ...
    void testUpdateAllocationValidationError() {
        ...
        assertThrows(
                HttpClientErrorException.UnprocessableEntity.class,
                () -> api.updateAllocation(TEST_CLIENT_API_KEY, allocationDTO.getId(), new UpdateAllocationDTO().subject(null))
        );
    }
}
